...
export function addQuestionMapping(output: Mapping, questions: string[], parameters: any, surveyTemplate: {survey: any, hashSurvey: Map<string,any>}, quest: string, i: number, jsonDataset: any, questionsMapping: Map<string, number>): void {
  ...
  if(parameters.questionTypes.filter((qType: any ) => qType?.type === row.Payload.QuestionType)) {//checks if this row's question type is in the paramters document
    ...
    //fetchQSF: assings the answer mapping
    ...
    //applyies the remapping from the choice order array
    ...
    if(questTypeParam.fetchDataset) {
      let uniqueAnswers = new Map<String, any>();
