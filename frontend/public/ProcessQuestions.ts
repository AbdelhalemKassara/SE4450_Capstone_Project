...
//console.log("hi " + mappingFile)
//console.log("hi " + qsfFile)
    ...
    console.log(ageBracs);

    //create the new answer mappings
    let newAnswerMapping: answersMapping = {};

    let birthYear = Number(ageBracs[0].minBirthYear)+1;
    
    if(qsfFile.getShowBirthYear()) {
      birthYear = qsfFile.getSurveyYear() - birthYear;
      }

    newAnswerMapping["1"] = {Display: `younger than ${ageBracs[0].bracketName} (${birthYear}+)`};
    

    ageBracs.forEach((ageBrac: AgeBracket, i: number) => { //creates the new answer mapping
      newAnswerMapping[(i+2).toString()] = {Display: `${ageBrac.bracketName} (${qsfFile.getShowBirthYear() ? ageBrac.minBirthYear : qsfFile.getSurveyYear() - ageBrac.minBirthYear
      } - ${i === 0? qsfFile.getShowBirthYear() ? qsfFile.getMaxBirthYear() : qsfFile.getSurveyYear() - qsfFile.getMaxBirthYear()
        : qsfFile.getShowBirthYear() ? Number(ageBracs[i-1].minBirthYear)-1 : qsfFile.getSurveyYear() - Number(ageBracs[i-1].minBirthYear)-1
        })`}//+2 because we are starting from 1 and for born before oldest bracket
    });

    newAnswerMapping[(ageBracs.length + 2).toString()] = {Display: `older than ${ageBracs[ageBracs.length-1].bracketName} (${qsfFile.getShowBirthYear() ? 
      Number(ageBracs[ageBracs.length-1].minBirthYear)-1 : (qsfFile.getSurveyYear() - Number(ageBracs[ageBracs.length-1].minBirthYear)-1) 
      }-)`};
