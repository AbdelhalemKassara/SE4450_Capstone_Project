...
const MapComponent = ({ mapData, mapType, setSelectedRiding }) => {
  ...
  const lowMultipliers = [0, 0.2, 0.4, 0.6, 0.8];
  ...
  useEffect(() => {
    if (mapData) {
      ...
      datasetQ.getJSONPolygonFile("province").then((polygon: any) => {
        ...
        setProvinceMapData({
          ...provinceMapData,
          features: [...updatedProvinceMapData],
        });
      });
      datasetQ.getJSONPolygonFile("riding").then((polygon: any) => {
        ...
        setRidingMapData({
          ...ridingMapData,
          features: [...updatedRidingMapData],
        });
        setMaxValue({
          ...maxValue,
          province: maxProvinceCount,
          riding: maxRidingCount,
        });
      });
    }
  }, [mapData]);
  ...
  useEffect(() => {
    let multiplier = lowMultipliers;
    if (maxValue[mapType] > 25) multiplier = multipliers;
    const multipliersMax = multiplier.map((val) => {
  ...
  return Object.values(mapData?.province).length >= 1 ? (
    <div id="map_box">
      <MapContainer center={[56.505, -92.09]} zoom={3} scrollWheelZoom={false}>
        ...
        <GeoJSON
          key={JSON.stringify(
            mapType === "province" ? provinceMapData : ridingMapData
          )}
